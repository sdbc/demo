...
char plt[131702],ciph[131720];
int i;
	for(i=0;i<sizeof(plt);i++) {
		plt[i]='A';
	}
    gettimeofday(&beg,0);
    crypto.encryptString(plt, ciph, sizeof(plt), AesCipher::OFB);
    gettimeofday(&end,0);
    printf("test OFB encrypt size=%d,time=%ld\n",sizeof(plt),interval(&beg,&end));
    outLen = crypto.calculateCipherLen(sizeof(plt));
    gettimeofday(&beg,0);
    crypto.decryptString(ciph, plt, outLen, AesCipher::OFB);
    gettimeofday(&end,0);
    printf("test OFB decrypt size=%d,time=%ld\n",sizeof(plt),interval(&beg,&end));
}
